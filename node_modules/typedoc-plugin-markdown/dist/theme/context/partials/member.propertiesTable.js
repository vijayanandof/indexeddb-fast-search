import { backTicks, htmlTable, strikeThrough, table, } from '../../../libs/markdown/index.js';
 ...
 */
export function propertiesTable(model, options) {
    ...
    declarations.forEach((property, index) => {
        ...
        const propertyName = backTicks(`${property.name}${property.flags.isOptional ? '?' : ''}`);
        ...
        if (hasDefaults) {
            row.push(this.helpers.getPropertyDefaultValue(property) ||
                backTicks('undefined'));
        }
        if (hasComments) {
